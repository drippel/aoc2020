...
object Day11 {
  ...
  def part1( grid : Grid ) : Int = {
    ...
    def innerPart( current : Grid, last : Option[Grid], steps : Int ) : Int = {
      
      if( gridEquals( current, last ) ) {
        // done - lets count
        current.toString.count( _ == '#' )
      }
      else {
  ...
  def gridEquals( g1 : Grid, g2 : Option[Grid] ) : Boolean = {
    g2 match {
      case Some(g) => { g1.toString.equals(g.toString) }
      case None => false
    }
  }
  
  def calcNextGrid(grid: Grid) : Grid = {
  ...
  def calcNextGrid2(grid: Grid) : Grid = {
